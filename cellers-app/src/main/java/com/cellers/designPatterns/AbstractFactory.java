package com.cellers.designPatterns;

import java.io.IOException;
import java.util.Scanner;

//as much as Provider like netflix, comcast, verizon, (widening your families)
//one vending machine has drinks,chips,water,cookies,candys (coke pepsi )-> factory design pattern
...
//Abstract Facotry is a factory of factories
...
abstract class Loan {
	...
	public void calculateLoanPayment(double loanamt, int years) {
		...
		n = years * 12;
		rate = rate / 1200;
		mortage = Math.round((((rate * Math.pow((1 + rate), n)) / ((Math.pow((1 + rate), n)) - 1)) * loanamt)*100)/100;
...
class BankFactory extends AbstractFactory {
	...
	public Bank getBank(String bank) {
		if (bank == null) {
			return null;
		} else if (bank.equalsIgnoreCase("BOA")) {
			return new BOA();
		} else if (bank.equalsIgnoreCase("WellsFargo")) {
			return new WellsFargo();
		} else if (bank.equalsIgnoreCase("PNC")) {
			return new PNC();
		}
		return null;
	}
